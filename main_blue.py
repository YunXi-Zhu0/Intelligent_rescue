...
from x3pcodes.balltracking_blue import *  # 包含 model_inference, max_area, target_choose, if_in_range, go_to_block_test2, etc.
...
def main(model_path, classes_name_path, cap):
    ...
    while isRun:
        ...
        while not judge_to_block:
            ...
            # 计算各颜色目标的最大面积和质心
            blue_max, blue_centroid, yellow_max, yellow_centroid, black_max, black_centroid, image = max_area(class_ids, boxes, image)


            # 选择最大面积目标，返回目标质心、最新图像、目标类别、目标边界框以及目标当前宽度
            _, max_centroid, image, class_ids, boxes, current_box_width = target_choose(blue_max, blue_centroid, yellow_max, yellow_centroid, black_max, black_centroid, image)

            # 判断目标是否进入目标区域
            blue_in_range, yellow_in_range, black_in_range = if_in_range(image, blue_centroid, yellow_centroid, black_centroid)
            judge_to_block = blue_in_range or yellow_in_range or black_in_range
            ...
            if max_centroid is not None:
                ...
                if flag:
                    continue
                # exit(1) #之后需要加入识别不到物体的小车运动策略, 暂时用exit退出程序
        ...
        while not judge_to_safety:
            ...
            # 获取蓝方安全区质心
            safety_blue_centroid = get_safety_blue(class_ids, boxes, class_list)

            # 调用 centroid_align 和 go_to_block
            if safety_blue_centroid is not None:
                if centroid_align(class_ids, boxes, image, safety_blue_centroid, screen_width=640, fov=120,

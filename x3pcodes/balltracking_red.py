...
def block_in_safety(class_ids, boxes, class_list, frame):
    """
    过滤掉 'red_safe', 'yellow_safe', 'black_safe' 标签，
    并且移除位于 safety_red 匂域内的 'red', 'yellow', 'black' 类物块。
    ...
    if (class_ids is None) or (boxes is None) or (class_list is None) or (frame is None or frame.size == 0):
        print("输入参数为空，无法处理！")
        return class_ids, boxes, class_list, frame  # 返回原本列表和图像

    safety_red_box = None  # 用于存储安全区的坐标 (x, y, w, h)

    # 遍历 class_ids 来找到 safety_red 区域
    for class_id, box in zip(class_ids, boxes):
        if class_list[class_id] == 'safety_red':
            safety_red_box = box
            break

    if safety_red_box is None:
        print("没有找到 safety_red 区域！")
        return class_ids, boxes, class_list, frame

    # 获取安全区的坐标 (x, y, w, h)
    x, y, w, h = safety_red_box
    ...
    for class_id, box in zip(class_ids, boxes):
        ...
        # 如果是 'red_safe', 'yellow_safe', 'black_safe' 标签则跳过
        if class_name in ['red_safe', 'yellow_safe', 'black_safe']:
            continue
        
        # 判断物块是否在 safety_red 区域内
        if class_name in ['red', 'yellow', 'black']:
...
def max_area(class_ids, boxes, frame):
    ...
    并在图像上标记这些框。对已经进入安全区的框（如带有 red_safe, yellow_safe, black_safe 的标签）不进行计算。
    ...
    return:
        red_max: 红色框的最大面积
        red_centroid: 红色框最大面积对应物体的质心坐标 (x, y)
        yellow_max: 黄色框的最大面积
    ...
    # 安全区类别
    safe_classes = {"red_safe", "yellow_safe", "black_safe"}

    # 初始化每种颜色的最大面积和质心
    max_areas = {"red": 0, "yellow": 0, "black": 0}
    centroids = {"red": None, "yellow": None, "black": None}
    max_boxes = {"red": None, "yellow": None, "black": None}

    # 检查输入是否为空或类型不匹配
    if not isinstance(boxes, (list, np.ndarray)) or not isinstance(class_ids, (list, np.ndarray)):
        return max_areas["red"], centroids["red"], max_areas["yellow"], centroids["yellow"], max_areas["black"], centroids["black"], frame
    ...
    for i, box in enumerate(boxes):
        ...
        class_name = str(class_id)  # 获取当前类别的名称或 ID
    ...
    # 提取不同颜色的最大面积和质心
    red_max, red_centroid = max_areas["red"], centroids["red"]
    yellow_max, yellow_centroid = max_areas["yellow"], centroids["yellow"]
    black_max, black_centroid = max_areas["black"], centroids["black"]

    return red_max, red_centroid, yellow_max, yellow_centroid, black_max, black_centroid, frame



def target_choose(red_max, red_centroid, yellow_max, yellow_centroid, black_max, black_centroid, frame):
    """
    ...
    areas_and_centroids = [
        (red_max, red_centroid, 0),  # 红色对应类别 0
...
def if_in_range(frame, red_centroid, yellow_centroid, black_centroid):
    """
    判断红色、黄色、黑色质心坐标是否在指定范围内，并实时显示视频流。
    
    param:
        frame: 实时视频帧。
        red_centroid: 红色框最大面积的质心坐标 (x, y) 或 None。
        yellow_centroid: 黄色框最大面积的质心坐标 (x, y) 或 None。
        black_centroid: 黑色框最大面积的质心坐标 (x, y) 或 None。

    return:
        red_in_range: 红色质心是否在指定范围内 (True/False)。
    ...
    # 定义颜色映射
    color_bgr = {"red": (0, 0, 255), "yellow": (0, 255, 255), "black": (0, 0, 0)}

    # 判断红色质心是否在范围内
    if red_centroid:
        rx, ry = red_centroid
        red_in_range = x_min <= rx <= x_max and y_min <= ry <= y_max
        color = (0, 255, 0) if red_in_range else (0, 0, 255)
        cv2.putText(frame, f"Red: {red_in_range}", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
    ...
    # 返回不同颜色是否在范围内
    return red_in_range, yellow_in_range, black_in_range
...
def get_safety_red(class_ids, boxes, class_list):
    """
    提取与 'safety_red' 标签对应的框的质心。
    ...
    return:
        safety_red_centroid: safety_red 标签对应的框的质心坐标 (x, y)
    """
    safety_red_centroid = None  # 初始化返回的质心坐标为 None
    ...
    for class_id, box in zip(class_ids, boxes):
        ...
        # 检查是否是 'safety_red' 类别
        if class_name == "safety_red":
            x, y, w, h = box
            # 计算质心坐标
            centroid_x = x + w // 2
            centroid_y = y + h // 2
            safety_red_centroid = (centroid_x, centroid_y)
            break  # 找到第一个 safety_red 后退出循环

    return safety_red_centroid
            ...
            if class_ids is not None and confidences is not None:
                ...
                if len(class_ids) > 0 and len(confidences) > 0:
                    for idx, confidence in zip(class_ids, confidences):  # 使用zip将class_ids和confidences配对
                        if confidence > 0.3:  # 根据设定的置信度阈值判断
                            # 获取对应的类别名称
                            class_name = class_list[idx]
                            # 判断目标是否为红色、黄色或黑色
                            if class_name.lower() in ['red', 'yellow', 'black']:  # 将蓝色改为红色
                                print("检测到目标为红色、黄色或黑色，停止拍照。")
                                flag = True    
            print("未检测到目标，继续拍照。")
                ...
                if len(class_ids) > 0 and len(confidences) > 0:
                    for idx, confidence in zip(class_ids, confidences):  # 使用zip将class_ids和confidences配对
                        if confidence > 0.3:  # 根据设定的置信度阈值判断
                            ...
                            # 判断目标是否为红色
                            if class_name.lower() == "safety_red":  # 将蓝色改为红色
                                print("检测到目标为红色，停止拍照。")
                                flag = True    
            print("未检测到目标，继续拍照。")
